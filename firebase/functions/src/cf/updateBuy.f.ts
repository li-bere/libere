...
export const updateBuy = functions.https.onCall(async (data, context) => {
  console.log("Update buy called");
  console.log(data);
  if (!context.auth) {
  ...
  if (!data.transactionId || (!data.isAccepted && !data.isRejected)) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "Bad data present"
    );
  ...
  try {
    ...
    if (data.isAccepted) {
      ...
      const notification = {
        ...
        body: `Your request has been accepted by shop ${shopData.name}`,
    ...
    console.log('end notificaton');
      ...
      // const rejectedTransactionDeleted = await db.collection("transactions").where('status', '==', 'REJECTED').get();
      // rejectedTransactionDeleted.forEach((deleteTransaction) => {
        
      // });
      console.log('this is the place where we set notification');
      const notification = {
        title : 'Buy Bottle Request',
        body: `Your request has been rejected by shop ${transactionData.shopName}`,
        transactionId: transactionData,
        click_action: 'FCM_PLUGIN_ACTIVITY',
        data: {
          redirectTo: '/user-tabs/transactions'
