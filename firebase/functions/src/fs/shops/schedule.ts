...
async function dailySched(){
    let rankings: any;
     const rankingsnapshot= await db.collection('Shop-rankings').get();
     rankingsnapshot.forEach(async function (doc) {
        rankings = doc.data();
        rankings.id = doc.id;
        await db.collection('Shop-rankings').doc(doc.id).update('dailyco2',0);
        await db.collection('Shop-rankings').doc(doc.id).update('dailypla2',0);
      });
...
async function monthSched(){
    let rankings: any;
     ...
     rankingsnapshot.forEach(async function (doc) {
        rankings = doc.data();
        rankings.id = doc.id;
        await db.collection('Shop-rankings').doc(doc.id).update('monthlyco2',0);
        await db.collection('Shop-rankings').doc(doc.id).update('monthlypla2',0);
      });
